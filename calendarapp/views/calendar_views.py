            ...
            event_list.append(
                {
                    ...
                    "url_details": reverse(
                        "calendarapp:event_details", kwargs={"pk": event.pk}
                    ),
                    "url_edit": reverse(
                        "calendarapp:event_edit", kwargs={"pk": event.pk}
                    ),
                    "url_delete": reverse(
                        "calendarapp:event_delete", kwargs={"pk": event.pk}
                    ),
                    ...
                    "allDay": int(
                        event.all_day
                    ),  # uses int() so that the boolean gets translated to JS boolean (0/1)
